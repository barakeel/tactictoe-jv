...
\paragraph{Two ways of representing a formulas.}\todo{RK: Actually I don't know what two ways you mean...?}
...
Considering concrete implementation details now, a \emph{tactic} is a function that takes a goal (or proof obligation) and returns a list of goals together with a validation function.
\todo{flesh the below out}
Input of the prediction: goal If a tactic produces no goals then its input goal is solved.
...
We define the children of a node $a$ by:
\[\mathit{Children}(a) = \bigcup_{g \in G(a)} \mathit{Children}(g) \]

And the descendants of $a$ by:
\begin{align*}
...
let fun RWTAC thms = SRW_TAC [] thms in
  RWTAC [fetch "pred_set""countable_def", fetch "pred_set" "INJ_DEF"] THEN
...
Its implementation,
\tactictoe, achieves an overall performance of 66.4\% theorems on 7164
theorems
