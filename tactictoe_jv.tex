...
which it is possible, but not common, to write a proof as a sequence of 
...
and applies tactics that transform the proof state until it is solved (or until 
giving up).
...
starting one.
This gives rise to a machine learning problem: can we learn a mapping from a 
given proof state to the next tactic (or sequence of tactics) that will 
productively advance the proof towards a solution?
...
If the developer gives the theorem a name then we refer to it as a 
...
construction.
Because validation functions are only important to check the final 
proof, we
...
The contributions of this extension and their effect on our system are:
...
\item Proof guidance required by MCTS is given by predictors presented in 
Section~\ref{s:prediction} for
...
success rate on the standard library. And detailed comparisons between 
\tactictoe and \eprover on the different type of problems are reported in 
...
proving tool chains. Functions are represented by rectangles and objects by 
ellipses.}
\end{figure}
...
in Section~\ref{sec:recording}. Based on such training examples, we
  ...
  guidance for the proof search algorithm presented in 
  Section~\ref{sec:proofsearch} 
  which explores different proof paths in the proof tree defined in 
...
we can give a description of the proof search algorithm sooner. This order is 
...
presented in Section~\ref{s:prediction}.
...
search tree is depicted, with goals represented by circles, nodes by rectangles 
and tactics by arrows.
...
We call a goal (respectively node) \textit{unsolved} if it is not in the set of 
solved
...
\item It is not a parallel step, i.e., there does not exist $t'$ in $T(g)$ 
...
\cite{ckju-pxtp13}. \todoi {Cezary: Josef says sixth power is not in this 
citation}.
...
here instead of neural networks trained through reinforcement learning
as e.g. in AlphaGo Zero~\cite{silver2017mastering}.
...
The number $\mathit{Failure}(a_i)$ is the number of failures that occurred
...
This happens only at the beginning of the \holfour library, where the training 
data
...
output of tactics. Theoretically, a list of goals is interesting if we can find 
a short proof for it. We could estimate the length of a proof from previous 
proof searches. However, this
...
declare a list of goals $l$ to be \textit{positive} if it is has been produced 
...
\todoi{maybe the paragraph can be generally proof strategy languages including ltac and eisbach}
