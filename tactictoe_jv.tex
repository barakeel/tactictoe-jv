...
\begin{definition}\label{def:desc}(Children, Descendant)\\
We now define what are the children and the descendants of a node $a$ in a 
search tree $\mathfrak{T}$. The children of a relative to a goal $g$ in $G(a)$ 
is the set of nodes produced by tactics applied to $g$:
  \[Children(a,g) = \lbrace A(g,t)\in \mathbb{A}\ |\ t \in T(g) \rbrace \]

From that, we define the children of a node $a$ by:
  \[Children(a) = \bigcup_{g \in G(a)} Children(a,g) \]

And descendants of $a$ by transitive closure:
\begin{align*}
...
\todo{make definition in preliminaries of active node and descendant?}
...
prior node evaluation for MCTS.
...
the previously required forcing.
\end{remark}
...
\todoi{mention productive tactics}
Distances between goals produced by the tactic predictor are hard 
to translate into a prior tactic policy, so we rely solely on the order of 
tactics to estimate the probability with which each tactic should be tried.
Let $t_0,\ldots,t_n$ be the list of preselected tactics ordered by their 
...
\[PriorPolicy(a_i) = PriorPolicy(t_i) = PriorPolicy(i) = (1 - c_{policy})^{i} * 
c_{policy}\]

Widening policy for a node $a$.

\[WideningPolicy(a) = \]

The first term of the multiplication is the probability that none of the better 
ranked tactic is selected. In fact, these probabilities are never used directly 
and only serves as a guidance to our deterministic node selection algorithm 
over many MCTS step.
...
explore increases. To decide which node to extend next, we proceed through the 
node selection algorithm. Starting from the root node, we repeatedly chose the 
children relative to the open goal with the highest value. 

The value of a $a_i$ with respect to a search tree is determined by:
...
The exploitation term $CurEvaluation(a_i)$ is the average of the evaluation of 
all MCTS step that went through $a_i$ (itself included) in the current search 
tree.
...
where $Failure(a_i)$ is the number of MCTS steps that went through $a_i$ which 
ended in a failure.

To allow the algorithm to select nodes that 
are not leafs, we also assign a value to the possibility of stopping during the 
descent of the tree. This value has an exploitation equal to 0 since it was 
never explored and is given a prior policy equal to $PriorPolicy(n)$ where $n$ 
is the 
number of 
tactics already tried on the open goal if there is one and $PriorPolicy(0)$ if 
there is 
none.

\subsection{Node extension}
Once a node is selected, we first check if it is an active part of the search 
tree (not a solved node . 

If the selected node is not active, then the result of node extension is a 
failure.
Otherwise, we apply the best untested tactic on the open goal $g$ of this node  
according to the prediction order for $g$.


we apply the next tactic in the prediction order for 
the open goal of this node.

The result of the extension step is either a failure or a list of goals $l$.
From $l$, we can assess if the tactic was productive. 
If not, we also return a failure.
Otherwise, the list of goals $l$ is used to create a new node in the search 
tree. Give precise definition. 

order of predicted tactics.

During the creation of node, we predict tactics for the open goals

\subsection{Backpropagation}

If node extension resulted in a failure, we increment the failure field of the 
selected node and its parents. If node extension was successful, we 
backpropagate the evaluation of the new nodes to their parents.
In both cases, we increment the number of visits of all nodes traversed.
This changes induce an update on the current evaluation and current policy for 
each node. And the process loops back to the node selection step.
...
premises for \metis, giving it a slightly higher timeout than other tactics and 
always trying it first on each node. These modifications only induce a
...
and calls \eprover on a version of the problem (premises + goal) translated to 
first order. We then extract the premises that were required in the \eprover 
proof and reconstruct it with \metis. Giving a special status to external 
prover is really necessary as external provers calls do not appear in human 
...
computationally, they are run in parallel and asynchronously. This 
...
%timeout.
%And the strongest external provers according to past experiments .
%Where as...
%
%experiment with a small hammer tactic comprised of: a faster premise 
%selection algorithm and
%a short call (0.1 seconds + 16 premises optimized by the previous paper) to 
%the 
...
%
%This forced behavior could often be rediscovered from the training data for 
%the 
...
%To integrate a general tactic that requires a higher time out such as 
%\eprover, 
