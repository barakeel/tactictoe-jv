...
\todo{TG : comparison with hammers in usability}
...
classifier~\cite{ckju-pxtp13,DudaniS76} to predict three different objects: 
 goals, theorems and list of goals. 

We first introduce
specificities associated with the prediction of each object: the dataset from
which objects are selected and the different use cases for the 
prediction. We then details how we predict objects suitable for the current 
goal by comparing their features with the feature of the goal.


\paragraph{Tactics}
Given a goal $g$, the classifier selects a list of input goals
ordered by their similarity to $g$. Since each input goal is associated 
with a tactic in our database, the ordered list of goals induces a ordered  
list of associated tactics which gives us a prior policy function for  MCTS. 
And tactics are tried in this order during the 
...
Whenever we mention that we want to predict a goal, we 
actually intend to predict its associated tactics.
...
of the policy and adapt it accordingly. 


\paragraph{Theorems}
Because we may augment the dataset of tactics with abstracted tactics where 
list of theorems have been erased, we need to find a way to instantiate them.


This algorithm is also useful for selecting premise to ATPs. (see 
Section~\ref{sec:atp})


Given a open goal $g$, the classifier selects a list of theorems ordered by 
their 
similarity to $g$.
A dataset of theorems is constructed from the \holfour database and local 
theorem values.
We predict theorems most similar to the goal for instantiate list of theorems 
that appears as arguments of abstracted tactics (see Section 
\ref{sec:synthesis}). A list of 
predicted theorems 
are also given as input to ATPs. The number of predictions is 16 for tactic 
arguments and \metis, but is increased to 128 for \eprover.



\paragraph{List of goals}
The dataset of learned list of goals is compiled during orthogonalization.
This set is separated into positive and negative examples.
Given a list of goals outputted by a tactic,
we compute the closest lists of goals in our dataset. The ratio of positive 
examples from the total number of examples gives us a prior evaluation function 
for MCTS (see Section~{}).
...
are, the more similar two goals should be. This can be realized by 
calculating TF-IDF weights for each feature~\cite{Jones72astatistical}.
We additionally raise the weights to the sixth power giving even more credence 
to rare features which was found experimentally optimal in \cite{cezary_cite}.
...
Making prediction for an object $o$ consists sorting a set of objects 
by their similarity to $o$.

We use $sim_1$ for goals and theorems and $sim_2$ for list of goals.
The similarity $sim_1$ is used for predicting a tactic (via goals) because if a
...
The predictions algorithm is called many times during proof search to chose 
suitable tactics, instantiate tactic arguments by terms and theorems and
estimate the provability of a list of goals.
In order to speed up the predictions, we preselect 500 features vectors of 
...
of theorems. We choose this very common type because perfect instantiation of 
those arguments would lead to the largest improvements (see 
Section~\ref{sec:perfect_exp}). We plan to 
extend our algorithm to term which is another frequent argument types.
But term prediction have been so far proven to weak to positively 
...
solved node and is given in Definition~\ref{def:solved_goal}. An open goal is 
goal that 
...
\begin{tabular}{lll}
\toprule
 PriorEvaluation & $c_{exploration}$ & Solved \\
\midrule
...
\bottomrule
\end{tabular}
\caption{\label{tab:search_parameters}}
%on the server
\end{table}
...
with tactics from the proof of the tested theorem.} 
\end{table}
\todo{good title for this table Additional features}
...
%It is also the first step toward self-reinforcement learning. 
