...
%To automate proof search strategies a user can implement programming language 
functions that correspond to more involved proofs.
%Functions that correspond to backward proofs, called tactics, are combined 
with each other with the help of tacticals (functions that takes
%tactics as arguments).
%Such a combination of tactics will be called a \emph{proof script} when it 
...
omit them during the description of proof search algorithm. We denote $t(g)$ 
the lists of goals produced by the application of a tactic $t$ to a goal 
$g$.
...
We first investigate how \tactictoe and \eprover scales as the search time 
grows in Figure~\ref{fig:c1}.
In 10 seconds, \tactictoe solves 90 percent of the problem it can 
...
their string representation and therefore we can avoid
