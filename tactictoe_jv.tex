...
can write for the first example a single new tactic encoding this procedure: 
...
arguments with placeholders \texttt{term} and \texttt{thm}: 
  ...
  thm}\]
Finding a witness to instantiate an existential formula is difficult because it 
is likely that this witness may not have been proposed before and there is no 
...
variable of the goal. That is why we think that the first step toward term 
prediction is to select subterms of the goal ordered by their similarity with 
the term argument appearing in the original tactic. 
Side experiments using a tactic that tries the 16 closest subterms 
$[term1,\ldots,term16]$ to the goal with the procedure described in the 
previous paragraph 
shows that this simple term predictor improves the success rate of 
\tactictoe by another 4 percent. The tactic derived from the abstraction
\texttt{EXISTS\_TAC term} in this experiment is:
