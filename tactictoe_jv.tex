...
thus we can consider the list to be a set in our algorithm.
\end{itemize}
...
replace the
placeholders in $\hat{t}$. The instantiation of the first abstracted is thus
\texttt{REWRITE\_TAC [thm1,...,thm16]}.

\paragraph{Theorem}
Instantiating a single theorem is much harder than a list of theorems. Indeed,
the predictions needs to be very precise for the tactic to be effective.
For example, the tactic MPTAC only works if the theorem is an instance
of the precondition. The best option would be to have a train a theorem 
predictor that can adapt its predictions to the given tactic. However, this 
would require a stronger machine learning model than nearest neighbor algorithm.
Therefore, in order to mitigate the gap in the quality of the predictions, we 
propose to create a tactic that tries instantiations until one succeeds. (i.e 
by succeeding we mean produces a list of goal that does not contain the 
original goal)
...
Side experiments with term argument predictions 
show that trying to instantiate the tactic with subterms 
from the goal improve the success rate by another 4 percent. 
However, in the evaluation presented in this paper, the 
abstraction/instantiation method is evaluated only for the type of theorem list.
Moreover, the current argument predictions are only affected by the given goal, 
and we believe it would be beneficial to have more flexible predictors that can 
adapt to their predictions to the abstracted tactic.
...
% TODO: Add a sentence about try-hard

%Comparatively, in \holfour, new tactics can be written by end-users in the 
%implementation 
%language \sml. Therefore, the design of a meta language was 
%not necessary as every proof method is a \sml function. 
%Using this language, the authors 
%created a default search strategy ``try-hard'' that includes calls to 
%\sledgehammer, and that outperforms \sledgehammer.
