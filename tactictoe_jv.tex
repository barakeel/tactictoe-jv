...
%We first discuss parsing proof scripts and identifying 
%individual tactic calls. We next show how to record tactics calls are recorded 
%with their corresponding goals using a modified proof script.
%The recorded data which consists of tactic-goal pairs creates the raw 
%knowledge 
%necessary for our tactic selection algorithm.
...
\item
It is probably difficult (even impossible) to transfer values 
...
of these steps in a running example that is representative of a typical proof 
script. And we comment on the main steps that transforms the original proof 
script into a modified version that is able to record the code of a tactic and 
the goal(s) it processed during the proof. We omit more involved constructions 
...
is of type tactic and that an infix guard is not a top symbol of the 
expression.
...
The function save(stac,goal) save the pair to disk increasing the number of 
tactic-goal pairs in our database.
...
%$goal$. And the features $features$ of the goal $goal$ are extracted and 
%hashed 
...
%
%To avoid the computational cost of re-proving every theorems at the begining 
%of 
...
%\sml file where those binders are declared. And for simplicity sake, all 
%theory 
...
combinators.
\todoi{Define tactic combinators?}
...
\todoi{Not sure if this reference is appropriate anymore}
