...
library show that TacticToe is able to re-prove 66.6\% of 7123 theorems in less 
than 60 seconds. 
This is to be compared with the 42.8\% proved in the same amount of time by a 
a barebone version of TacticToe.

In the same condtion the ATP Eprover proves ..., when it is run in 
auto-schedule mode with 128 lemmas predicted and 
translated by HolyHammer.
\todo{to do eprover results running}
...
A tactical search tree is a sixtuple $(\mathbb{A},\mathbb{G},\mathbb{T},A,G,T)$ 
...
We assume that any set is numbered and that we can access a nth member of set 
...
\end{tikzpicture}
\end{center}
\caption{A node of a search tree and its relation with the children of one of 
its goal}
\label{fig:choice}
\end{figure}
...
Let $\mathbb{T}$ be a search tree. The set of solved nodes $\mathbb{A}^\infty$ 
and 
