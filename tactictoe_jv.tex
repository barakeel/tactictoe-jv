...
library show that TacticToe is able to re-prove 66.2\% of all 7164 theorems in 
less 
than 60 seconds. 
This is to be compared with the 42.9\% proved in the same amount of time by 
Eprover in auto-schedule mode with 128 lemmas predicted and 
translated by HolyHammer.
\todo{run experiments including proof scripts containing let
     run experiments without prove on 1/10th of the library.}
...
Once a node is selected, we applied the best not-yet-applied tactics to
the open goal this node. If the tactic is productive then the we create a new 
