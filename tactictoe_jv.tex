...
%component. 
We first explain how we compute the evaluation and policy functions 
from learned data. 
And then during the application of the MCTS algorithm, we show how 
node selection steps and node extension steps make the search tree
(described in Section~\ref{sec:prelim}) evolve.
...
\metis a special status. This arrangement consist in always predicting premises 
for \metis, giving it a slightly higher timeout than other tactics and always 
trying it first on each node. These modifications only induce a linear overhead 
on the number nodes as a general purpose prover does not produce more goal 
obligations.
...
predicted premises (128). Since those external calls to \eprover can be 
computationally expensive, we call it in parallel and asynchronously. This 
avoids slowing down \tactictoe's main loop. The number of asynchronous calls to 
an external provers that can run at the same time is limited by the number of 
