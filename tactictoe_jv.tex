...
Defintion of a tactic in \holfour. In this research, we will take the definition
...
\addplot table[x=length, y=proofs] {data/tactictoe_proof_length};
\addplot table[x=length, y=proofs] {data/original_proof_length};
\legend{scaled \tactictoe,original}
\end{axis}
\end{tikzpicture}
\caption{Number of original human proofs with a number $x$ of tactic units. 
         Since there is about two-third of the original proofs that have a 
         respective \tactictoe proofs, we scale the number of 
         \tactictoe proofs by a factor of $\frac{1}{0.662}$.}
\end{figure}

There are more in proportion \tactitoe small proofs than original human proofs.
