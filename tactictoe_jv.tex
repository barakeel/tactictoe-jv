...
prover. TacticToe learns from many human proofs which mathematical method is 
useful in particular proof situations.
...
results of \tactictoe and \eprover.
\end{abstract}
...
Many of the state-of-the-art interactive theorem provers (ITPs) such as
  ...
  Such tactics analyze the current proof state (goal and
  ...
  Thanks to the progress in proof 
  ...
  theorem provers(ATPs) such as \eprover~\cite{eprover}. The communications 
  between 
  ...
  tactics (Section~\ref{sec:proofdisplay}). The system is evaluated on a large 
...
This happens less than 10 times in the full-scale experiment. \todo{@TG: why don't we have the original tactics as options?}
...
\todo{@TG: decide which of the next sentences}
...
\todo{this paragraph was confusing for Lasse and Cezary}
...
only previously declared \sml variables (essentially tactics, theorems and 
...
by calling \metis for 2.0 seconds with the set of theorems necessary for 
...
notice duplicated proofs. Comparatively, our predictions comes from a much more
