...
$\mathbb{A}$. Its support $\mathit{Support}(A)$ is 
$\lbrace (g,t) \in \mathbb{G} \times \mathbb{T}\ |\ t \in T(g)
\rbrace$. And if $(g,t) \in \mathit{Support}(A)$, then $t(g) = G(A(g,t))$. In 
other words, the output $t(g)$ is exactly the list
of goals contained in the node produced by $A$ from $(g,t)$.
\item It is acyclic, i.e. a node cannot be a descendant (see 
