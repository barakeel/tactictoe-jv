...
evaluation. One one hand, our predictor learning abilities is
limited. On the other hand, the evaluation function was designed to estimate
the provability of a goal. A more accurate evaluation could be based on an
estimation of the length of the proof required to close a goal.
...
To conclude, we also demonstrate the capabilities of \tactictoe on a goal
that did not have a proof in \holfour. 

\todoi{commented out because the theorem exists in hoL4}.
%The first goal, in Example~, states that the set of 
%natural numbers is countable. \tactictoe starts from the definition of 
%countability and injection then instantiate the result with the trivial 
%injection and finalize the proof using the \metis. Here,
%\tactictoe is able to find an instantiation for an existential quantifier 
%because it is a common one. If a more complicated term was needed, \tactictoe 
%would try to find an alternative proof that do not rely on an original
%instantiation. This weakness could be mitigated by the introduction of term 
%prediction in a future version of \tactictoe.

%\begin{example}
%\begin{lstlisting}[language=SMLSmall]
%countable (UNIV:num set)
%let fun RWTAC thms = SRW_TAC [] thms in
%  RWTAC [countable_def, INJ_DEF] THEN Q.EXISTS_TAC `\\x.x` THEN METIS_TAC []
%end
%\end{lstlisting}
%\end{example}

In Example~, \todoi{TG to RK: I am not sure what the count 
constant is doing.}
