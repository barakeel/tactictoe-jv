...
that they can keep their infixity after globalization. For simplicity, the 
...
three theorems in our script. The internal database of \holfour is available 
across \holfour developments. 
...
The function \texttt{save} writes the tactic-goal pairs to disk increasing the 
number of entries in our database of tactics. The function 
\texttt{tactic\_of\_sml} interprets \sml code \texttt{stac}. The tactic is then 
applied to the input goal which replicates the original behavior.
