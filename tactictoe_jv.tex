...
influence the behaviour of the MCTS algorithm.
Then we describe the five steps of the MCTS algorithm and their effect on 
the search tree (described in Section~\ref{sec:prelim}): 
initialization, node selection, node extension, backpropagation and resolution. 
...
\[PriorPolicy(t_i) = PP(i) = (1 - c_{policy})^{i} * c_{policy}\]
...
children with the highest value. 


The value of a $a_i$ with respect to a search tree is determined by:
\[Value(a_i) = CurrentEvaluation(a_i) + c_{exploration} * Exploration(a_i)\] 
...
This evaluation also include failures of the selection process.

 It 
is also called the current evaluation of $a_i$.   
\[CurrentEvaluation(a_i) = 
  \sum_{b \in Desc(a_i)} \frac{PriorEvaluation(b)} {(card Desc(a_i) + card 
  (Failure(a_i))}\]

where $Failure(a_i)$ is the number of run that went through $a_i$ that ended in 
a failure.
...
never explored and is given a prior policy equal to $PP(n)$ where $n$ is the 
number of 
tactics already tried on the open goal if there is one and $PP(0)$ if there is 
none.

\subsection{Node extension}
Once a node is selected, we applied the best not-yet-applied tactics according 
to the prediction order for the open goal of this node.
If no open goals exists we apply it to the first pending goal (according to 
some arbitrary order). If the node is a solved node or (a descendant of a 
solved node) then 
If the tactic is productive then the we create a new 
node and calculate and save its prior evaluation. Otherwise we evaluate the 
output of the tactic to 0 and backpropagate the failure to the ancestries.

Maybe should move this definition in static description.

To prevent tactics from looping, we interrupt tactics after
0.01 to 0.1 seconds. The optimal tactic timeout varies in function of the 
search time limit.
...
Number of failed selection and 
the number of visits are backpropagated to the ancestors

which update the current evaluation in each node.
...
\item It exceeds a given time limit.
\end{itemize}

The second case happens very rarely (less than 10 times in the full scale 
experiment). So our experiments report only the number of proofs found relative 
to the number of attempted problems.
\end{definition}
