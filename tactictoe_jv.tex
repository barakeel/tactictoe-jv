...
a short call (0.1 seconds) to the internal prover \metis~\cite{metis} or a
...
these goal are deactivated terminating associated threads. This frees space for 
more pending threads associated to newly created nodes to start.
...
(Todo: change next figure to an histogram)
\begin{figure}[h]
\centering           
\begin{tikzpicture}[scale=1]
\begin{axis}[
...
Future work:
  Promising techniques that could not be evaluated. Orthogonalization of 
  theorems~\cite{ckju-jsc15}. Term predictions.


Cite in future works.
