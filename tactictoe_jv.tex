...
In this way, tactics implement the plumbing of backwards, goal-directed, proof construction.
\todo{do not mention validation? RK: it depends whether these details are helpful for understanding or not}
...
, readability, maintainability and verification speed. We study these
properties
in three examples taken from our full-scale experiment. The time needed by
\holfour to check a proof is shown in parentheses.
...
In
contrast, \tactictoe finds a much smaller proof that expands the
definition of injectivity and surjectivity and calls \metis. However, the
\tactictoe's script does take much longer to check due to the proof search
happening inside
\metis.
