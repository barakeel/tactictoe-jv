...
that takes a goal (or proof obligation) and returns a list of goals together 
with a validation function. \todoi{do not mention validation?}
...
Starting from the parent node, we can construct children of a goal of
...
In Example~\ref{ex:cs1}, the uniqueness of the greatest common divisor is 
proven. The human developer recognizes that it follows easily from two theorems.
...
goal and split the goal into multiple cases until goal obligations are similar 
...
that $f o g$ is the identity function. The human proof is quite complicated. In 
contrast, \tactictoe finds a proof that first expand the definition of 
injectivity and surjectivity and then call \metis. However, the simpler script 
does take much longer to execute due to the proof search happening inside 
\metis.
...
In Example~\ref{ex:cs3}, we prove a theorem about lists which is domain where 
\tactictoe excels compared to ATPs. 
Given two list $l1$ and $l2$ where the length of $l1$ is less than a natural 
number $n$, the theorem states that performing an update on the $n^{th}$ 
element of the concatenation of $l1$ and $l2$ is the same as
performing an update on the $m^{th}$ element of $l2$ where $m = n - 
\mathit{length} l1$ followed by concatenation. Again, \tactictoe's proof is 
...
in the inductive hypothesis. It finalizes the proof using the definition of 
...
original human proofs in the \holfour repository.
...
In Example~\ref{ex:cs4}, we demonstrate the capabilities of 
\tactictoe on a goal that did not have a proof in \holfour. 

\todoi{TG to RK: I am not sure what the count 
constant is doing. Feel free to add more examples.}
