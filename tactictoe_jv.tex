...
[thm1,\ldots,thm16]$, we create 16 instantiated tactics and try them in the 
order given by the predictor, and we return the result of the first successful 
tactic and produces new goals. For readers familiar with \holfour tacticals, we 
give the single new tactic that can be created using this procedure on the 
first example:  
  \texttt{CHANGED\_TAC (MATCH\_MP\_TAC thm1) ORELSE ... ORELSE CHANGED\_TAC 
  (MATCH\_MP\_TAC thm16)}.
...
Moreover, the current argument predictions are only influenced by the input 
goal, 
and we believe that in the future it would be beneficial to have more flexible 
predictors that can adapt their predictions to each abstracted tactic.
