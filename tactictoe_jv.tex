...
\institute{Cezary Kaliszyk \at
Department of Computer Science, University of Innsbruck,
%Technikerstr. 21a/2,
Innsbruck, Austria\\ \url{bla@bla}
\and
Karol Pak \at Institute of Informatics, University of Bialystok, Bialystok, Poland\\\url{bla@bla}}
...
{columns=fullflexible,
  keywords={THEN,val,open,store_thm,fun,fn},%
  frame=none,
  sensitive=true,
...
of the ITP HOL4. 
After learning in which situations tactics are effective from 
tactic-level proof and what kind of formulas are the results of effective 
steps, we implement a Monte Carlo tree search algorithm that explores the most 
promising tactic-level proof paths. Experiments on the \holfour standard 
library shows that TacticToe is able to re-prove 59\% of 7088 theorems in less 
than 10 seconds where as the ATP Eprover with auto-schedule
solves 29\%.
...
\todo{These 2 paragraphs are copied}
...
\sml function. Intermediate files storing the result of each step are not 
necessary anymore. So, our recording is more complete and fully 
automatized.
...
independently. Combinatorial explosion in the number of tactics 
...
comparing subtrees instead of leafs. (todo: A first successful evaluation 
function.)
\item The internal ATP \metis used during our proof search is complemented by 
calls to \holyhammer. For efficiency reasons, those calls are run in parallel 
and asynchronously.
\item First evaluation of \tactictoe and \holyhammer success rates relative to 
...
\section{Related Work}\todo{reorganise}
...
a work on lemma mining~\cite{ckju-jsc15} in \hollight. Theorems were mined from 
proofs recorded at the kernel level. Because the number of recorded lemmas was 
in millions, page ranking techniques was used as a filter to extract ``useful'' 
lemmas.
...
abstract syntax tree (AST). The AST is then transformed into a globalized AST
where every top-level value is made independent from
the context. \todo{some reason why this is needed? nice recording level} We 
...
This is algorithm is used to predict tactics in the preselection
phase of the algorithm. Similarly, dependencies between theorems are used to 
preselect theorems.

\subsection{Preselection}\label{sec:pretactic}
...
coverage $C(t)$ of a tactic $t$ by the number of times this tactic appears in 
$\mathfrak{F}$. Expressing this in a formula, we get:
  \[C(t) =_{def} card\ \lbrace (x,g)\ |\ (x,g) \in \mathfrak{F} \wedge x = t
  \rbrace  \]
...
A goal subsumption $S$ is a partial reflexive relation on a set of goals.\\
Given two goals $g_1$ and $g_2$, we note:
$g_1 \ge g_2 \Leftrightarrow_{def} (g_1,g_2) \in S$.\\
...
In our experiments, we take the minimal subsumption defined by $g_1 \ge g_2  
\Leftrightarrow_{def} g_1 =_{\alpha} g_2$ where $=_{\alpha}$ is 
$\alpha$-equivalence.
\end{definition}

We can naturally extend a goal subsumption to a set of goals in the following 
way.
...
predicted
tactics, $S$ for the tactic subsumption relation and $C$ for the coverage of 
...
that is able to replicate the effect of the original tactic (or do better if we 
woudl consider more complex goal subsumptions).



\section{Tactic Abstraction}\label{sec:synthesis}
...
which direction.
Our search algorithm takes inspiration from advanced tree search algorithm such 
that

The first necessary adaptation of the algorithm stems
from the fact that a proof is in general not a
path but a tree. This means that our search space has two branching factors: 
...
We will reuse the prediction

%Search Algorithm AlphaGo Zero uses a much simpler variant of the asynchronous 
%policy and
%value MCTS algorithm (APV-MCTS) used in AlphaGo Fan and AlphaGo Lee.
%Each node s in the search tree contains edges (s, a) for all legal actions a ∈ 
%A(s). Each edge
%stores a set of statistics,
%{N (s, a), W (s, a), Q(s, a), P (s, a)},
%where N (s, a) is the visit count, W (s, a) is the total action-value, Q(s, a) 
%is the mean action-value,
%and P (s, a) is the prior probability of selecting that edge. Multiple 
%simulations are executed in
%parallel on separate search threads. The algorithm proceeds by iterating over 
%three phases (a–c in
%Figure 2), and then selects a move to play (d in Figure 2).
%25Select (Figure 2a). The selection phase is almost identical to AlphaGo Fan 
%12 ; we recapitulate
%here for completeness. The first in-tree phase of each simulation begins at 
%the root node of the
%search tree, s 0 , and finishes when the simulation reaches a leaf node s L at 
%time-step L. At each
%of these time-steps, t < L, an action is selected according to the statistics 
%in the search tree,
%
...
%1 + N (s, a)
%b
%where c puct is a constant determining the level of exploration; this search 
%control strategy initially
%prefers actions with high prior probability and low visit count, but 
%asympotically prefers actions
%with high action-value.
%Expand and evaluate (Figure 2b). The leaf node s L is added to a queue for 
%neural network
%evaluation, (d i (p), v) = f θ (d i (s L )), where d i is a dihedral 
%reflection or rotation selected uniformly
%at random from i ∈ [1..8].
%Positions in the queue are evaluated by the neural network using a mini-batch 
%size of 8; the
%search thread is locked until evaluation completes. The leaf node is expanded 
%and each edge (s L , a)
%is initialised to {N (s L , a) = 0, W (s L , a) = 0, Q(s L , a) = 0, P (s L , 
%a) = p a }; the value v is then
%backed up.
%Backup (Figure 2c). The edge statistics are updated in a backward pass through 
%each step
%t ≤ L. The visit counts are incremented, N (s t , a t ) = N (s t , a t ) + 1, 
%and the action-value is updated
%to the mean value, W (s t , a t ) = W (s t , a t ) + v, Q(s t , a t ) =
%W (s t ,a t )
%.
%N (s t ,a t )
...
The third point is a partial attempt to prevent confluent branches. The general 
...
that has the shortest proof, and we could measure the likelyhood of being 
...
\section{ATP and Lemma Prediction Integration}
...
a short call to the internal prover \metis~\cite{metis}.

It is best tactic available but we do not have any data about it since it does 
not appear in any proofs. So as a first approximation, we 
artificially give it the best prediction scores on any goal and the best 
coverage. 
...
continue its proof in parallel of a ``hammer'' tactic call. Additional care 
needs to be taken after each proof step to close proofs solved by the general 
tactic. When the number of thread reaches a limit, a ``hammer'' tactic call 
should
...
\subsection{Methodology} \todo{is copied}
The evaluation imitates the construction of the library: For each theorem only 
the previous
human proofs are known. These are used as the learning base for the predictions.
To achieve this scenario we re-prove all theorems during a modified build of 
\holfour.
As theorems are proved, they are recorded and included in the training examples.
...
\subsubsection{Tactic synthesis}
Here, we test the benefit of tactic synthesis compared to the old version.
...
\subsubsection{Hammer parameters}
...
that 0.1 
seconds is a better time limit for reasoning with 16 premises. And the $D_{18}$ 
experiment reveals the disadvantage of unnecessarily increasing the timeout of 
\metis. This reduces the time available for the rest of the proof search, which 
...
solid, mark repeat=20, mark phase=10, black!100, mark=triangle*\\
solid, mark repeat=20, mark phase=10, black!75, mark=square*\\
solid, mark repeat=20, mark phase=10, black!50, mark=*\\
solid, mark repeat=20, mark phase=10, black!50, mark=o\\
}
...
\bottomrule
\end{tabular}
 \caption{\label{tab:stats}search statistics}
\end{table*}
...
from the proof. To obtain a shorter proof, the diagonalization coefficient is
reduced to encourage exploration in the width of the proof tree. Similarly by 
ponderating each tactic application by its time and setting the 
diagonalization  coefficient is set to a tenth of the timeout of a tactic, we 
can aim for faster proofs.

\paragraph{Minimizing tactic arguments}
One of the argument that may contain unnecessary information are lists.
We test if the minimized tactic has the same effect as the original one.
This is a generalization of the simplest method used for minimizing a list of 
theorems in ``hammers'' \cite{}. The fact that the lists are generally small 
...
The inductive case should require an arithmetic decision procedure as hinted by 
the human 
...
%  maxdepth: 7
%Time: 9.574603
%  inferstep time: 7.287698
%  node_find time: 0.135171
%  node_crea time: 1.674807
%    pred time: 0.335816
